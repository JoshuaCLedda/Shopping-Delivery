...
if ($reportType === 'orders') {
    ...
    $stmt = $conn->con->prepare("
        SELECT transaction.id, transaction.u_id, transaction.total_price,
            transaction.status, transaction.order_date, 
            CONCAT(users.f_name, ' ', users.l_name) AS Name
        FROM transaction
        LEFT JOIN users ON transaction.u_id = users.u_id
        WHERE transaction.order_date BETWEEN ? AND ? AND transaction.rs_id = ?
    ");
    $stmt->bind_param("ssi", $startDate, $endDate, $restaurant_id);
    $stmt->execute();
...
elseif ($reportType === 'sales') {
    ...
    $stmt = $conn->con->prepare("
        ...
        FROM transaction
        LEFT JOIN users ON transaction.u_id = users.u_id
        WHERE transaction.status = 'order_delivered' AND transaction.order_date BETWEEN ? AND ? AND transaction.rs_id = ?
    ");
